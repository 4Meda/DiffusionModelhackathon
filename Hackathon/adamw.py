...
def get_parser():
    ...
    parser.add_argument('--optim', default='adam', type=str, help='optimizer', choices=['sgd', 'adam', 'adamw', 'sgld', 'sam'])
    parser.add_argument('--lr_max', default=5e-4, type=float, help='learning rate')
...
for epoch in range(n_epochs):
    ...
    if loss_train < best_loss:
        ...
        model_path = 'ckpts/adamw/model_best.pth'
        torch.save(model.state_dict(), model_path)

    loss_list.append(loss_train)
    epoch_number += 1

model_path = 'ckpts/adamw/model_last.pth'
torch.save(model.state_dict(), model_path)
